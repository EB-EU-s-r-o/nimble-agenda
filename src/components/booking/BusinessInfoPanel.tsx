...
export function BusinessInfoPanel({ info, openStatus, nextOpening }: Props) {
  ...
  return (
    <div className="space-y-4 border-gold">
      {/* Open/Closed badge */}
      {openStatus &&
      ...
      {/* Weekly hours */}
      <div className="rounded-lg border border-border p-3 bg-[#00050f]">
        <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5 text-muted-foreground" /> Otváracie hodiny
        </h3>
        <div className="space-y-1">
          {hoursByDay.map(({ day, mode, intervals }) =>
          <div key={day} className="flex items-center justify-between text-xs bg-[#050000]">
      ...
      <div className="flex flex-wrap gap-2 bg-[#0a0800]">
          {info.quick_links.
        ...
        <a
          ...
          className="inline-flex items-center gap-1 text-xs hover:underline text-gold">
