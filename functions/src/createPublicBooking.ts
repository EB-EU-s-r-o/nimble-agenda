...
async function isEmployeeBookable(
  businessId: string,
  employeeId: string,
  employeeProfileId: string | null,
  allowAdminAsProvider: boolean
): Promise<boolean> {
  ...
  if (!employeeServicesSnap.empty) return true;
  if (!allowAdminAsProvider || !employeeProfileId) return false;
...
export const createPublicBooking = onRequest(
  ...
  async (req: Request, res) => {
    ...
    try {
      ...
      const bookable = await isEmployeeBookable(
        business_id,
        employee_id,
        employee.profile_id ?? null,
        (await db.doc(`businesses/${business_id}`).get()).data()?.allow_admin_as_provider === true
      );
      if (!bookable) {
