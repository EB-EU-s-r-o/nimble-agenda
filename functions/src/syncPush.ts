...
async function triggerNotification(appointmentId: string, businessId: string, eventType: "created" | "updated" | "cancelled") {
...
async function applyCreate(
  ...
  const apptId = (p.id as string) || db.collection("appointments").doc().id;
  ...
  if (hasConflict) return { ok: false, reason: "Slot already occupied" };
  ...
  if (customersSnap.empty) {
    ...
    await newCustRef.set({
      ...
      full_name: (p.customer_name as string) || "Walk-in",
      email: walkinEmail,
...
async function applyUpdate(
  ...
  if (p.start_at) update.start_at = p.start_at;
  if (p.end_at) update.end_at = p.end_at;
...
async function applyCancel(
  ...
  if (typeof apptId !== "string") return { ok: false, reason: "Appointment not found" };
  const ref = db.doc(`appointments/${apptId}`);
  const snap = await ref.get();
  const data = snap.data();
  if (!snap.exists || data?.business_id !== businessId) {
...
export const syncPush = onCall({ region: "europe-west1" }, async (request) => {
  ...
  const adminMembership = membershipSnap.docs.find((d) => ["owner", "admin"].includes(d.data().role as string));
  if (!adminMembership) throw new HttpsError("permission-denied", "Not a business admin");
  const businessId = adminMembership.data().business_id as string;
  const actions: OfflineAction[] = (request.data?.actions as OfflineAction[]) ?? [];
  const conflicts: Array<{ idempotency_key: string; reason: string }> = [];
  let applied = 0;
  ...
  for (const action of actions) {
    ...
    try {
      ...
      conflicts.push({ idempotency_key: action.idempotency_key, reason: (e as Error).message });
    }
  }
