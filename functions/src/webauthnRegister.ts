...
export const webauthnRegisterChallenge = onCall({ region: "europe-west1" }, async (request) => {
  ...
  const excludeCredentials = existingSnap.docs.map((d) => ({ id: d.data().credential_id, type: "public-key" as const }));
  return {
    ...
    timeout: 60000,
    attestation: "none",
    authenticatorSelection: { authenticatorAttachment: "platform", residentKey: "preferred", userVerification: "required" as const },
  };
});
...
export const webauthnRegister = onCall({ region: "europe-west1" }, async (request) => {
  ...
  const publicKey = request.data?.publicKey as string;
  const deviceName = request.data?.deviceName as string | undefined;
