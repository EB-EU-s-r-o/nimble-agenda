...
export const seedDemoAccounts = onCall({ region: "europe-west1" }, async (request) => {
    ...
    await db.doc(`memberships/${membershipsDocId(demoUid, businessId)}`).set({ profile_id: demoUid, business_id: businessId, role: "employee", created_at: new Date().toISOString() }, { merge: true });
