...
export const webauthnAuthenticate = onCall({ region: "europe-west1" }, async (request) => {
    ...
    if (!credentialId)
        throw new HttpsError("invalid-argument", "Missing credential");
    ...
    const profileId = passkey.profile_id;
    ...
    if (!email)
        throw new HttpsError("failed-precondition", "No email associated");
    const auth = getAuth();
